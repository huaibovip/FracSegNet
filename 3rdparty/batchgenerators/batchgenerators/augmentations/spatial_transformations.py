...
def augment_mirroring(sample_data, sample_seg=None, axes=(0, 1, 2), sample_dismap=None):
...
def augment_spatial_with_dismap(data, seg, dismap, patch_size, patch_center_dist_from_border=30,
                    do_elastic_deform=True, alpha=(0., 1000.), sigma=(10., 13.),
                    do_rotation=True, angle_x=(0, 2 * np.pi), angle_y=(0, 2 * np.pi), angle_z=(0, 2 * np.pi),
                    do_scale=True, scale=(0.75, 1.25), border_mode_data='nearest', border_cval_data=0, order_data=3,
                    border_mode_seg='constant', border_cval_seg=0, order_seg=0, 
                    border_mode_dismap='constant', border_cval_dismap=0, order_dismap=3, random_crop=True, p_el_per_sample=1,
    ...
    if dismap is not None:
        if dim == 2:
            dismap_result = np.zeros((dismap.shape[0], dismap.shape[1], patch_size[0], patch_size[1]), dtype=np.float32)
        else:
            dismap_result = np.zeros((dismap.shape[0], dismap.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                                    dtype=np.float32)
    ...
    for sample_id in range(data.shape[0]):
        ...
        if modified_coords:
            ...
            if dismap is not None:
                for channel_id in range(dismap.shape[1]):
                    dismap_result[sample_id, channel_id] = interpolate_img(dismap[sample_id, channel_id], coords, order_dismap,
                                                                        border_mode_dismap, cval=border_cval_dismap)
        else:
            if seg is None:
                s = None
            else:
            ...
            if dismap is None:
                ...
                d_map = dismap[sample_id:sample_id + 1]
            ...
            if random_crop:
                ...
                d_map, _ = center_crop_aug(d_map, patch_size, None)

            data_result[sample_id] = d[0]
            if seg is not None:
                seg_result[sample_id] = s[0]
            if dismap is not None:
                dismap_result[sample_id] = d_map[0]
