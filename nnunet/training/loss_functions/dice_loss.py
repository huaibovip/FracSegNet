...
def get_tp_fp_fn_tn_dismap(net_output,
                           gt,
                           axes=None,
                           mask=None,
                           square=False,
    ...
    if smooth_trans == True:
        if current_epoch < Tauo_st:
            ...
            temp_disMap_value = warm_para * warm_start_matrix + (
                1 - warm_para) * disMap
...
class SoftDiceLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None, disMap=None, current_epoch=None):
        shp_x = x.shape
        ...
        if disMap is None:
            ...
            tp, fp, fn, _ = get_tp_fp_fn_tn_dismap(
                x, y, axes, loss_mask, False, disMap, current_epoch)
...
class DC_and_CE_loss(nn.Module):
    ...
    def forward(self, net_output, target, disMap_weight=None, epoch=None):
        ...
        dc_loss = self.dc(net_output,
                          target,
                          loss_mask=mask,
                          disMap=disMap_weight,
                          current_epoch=epoch) if self.weight_dice != 0 else 0
        if self.log_dice:
            dc_loss = -torch.log(-dc_loss)

        ce_loss = self.ce(net_output,
                          target[:, 0].long(),
                          disMap=disMap_weight[:, 0],
                          current_epoch=epoch)
        ce_loss = torch.mean(ce_loss) # TODO
        if self.ignore_label is not None:
            ce_loss *= mask[:, 0]
